...
(function () {
    ...
    const match = URL.match(/https:\/\/(pbs\.twimg\.com\/media\/[a-zA-Z0-9\-\_]+)(\?format=|.)(jpg|jpeg|png|webp)/);
    // browsing an image URL
    if (match) {
        // webp change to jpg
        if (match[3] == 'webp') match[3] = 'jpg';
        // change it to obtain the original quality.
        if (match[2] == '?format=' || !/name=orig/.test(URL)) {
            window.location.replace(`https://${match[1]}.${match[3]}?name=orig`);
        }
    }
                ...
                for (let mutation of mutationsList) {
                    ...
                    if (target.className.includes('css-1dbjc4n')) {
                        ...
                        for (let i = 0; i < images.length; i++) {
                            let image = images[i];
                            let imgMatch = image.src.match(/https:\/\/(pbs\.twimg\.com\/media\/[a-zA-Z0-9\-\_]+)(\?format=|.)(jpg|jpeg|png|webp)/);
                            // Image url change to original quality.
                            if (imgMatch) {
                                if (imgMatch[3] == 'webp') imgMatch[3] = 'jpg';
                                if (imgMatch[2] == '?format=' || !/name=orig/.test(image.src)) {
                                    image.src = `https://${imgMatch[1]}.${imgMatch[3]}?name=orig`;
                                }
                            } else
